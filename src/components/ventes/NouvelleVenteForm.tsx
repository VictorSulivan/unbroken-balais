...
import FormExtras from "./FormExtras";   // <-- Import du composant découpé précédemment
...
export default function NouvelleVenteForm({ clients: initialClients, produits }: { clients: Client[]; produits: Produit[] }) {
  ...
  // Transforme le clientId unique en format compatible avec le composant partagé FormClients (tableau)
  ...
  return (
    <>
      ...
      <div className="space-y-6">
        {/* CLIENT (Réutilisation du composant FormClients adapté pour une sélection unique) */}
        <FormClients
          clients={clients} 
          selectedClients={clientSelectionFormat} 
          onSelectClient={(id) => setClientId(id)} 
          onRemoveClient={() => setClientId(null)} 
          onUpdateNbPersonnes={() => {}} // Non requis pour une vente simple
            ...
            <div className="border-t border-white/10 pt-4 space-y-2">
              {lignes.map((l) => (
                <div key={l.produitId} className="flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => setQuantite(l.produitId, 0)} className="text-white/20 hover:text-red-400 text-xs transition-colors">✕</button>
                </div>
              ))}
            </div>
          )}
        ...
        {/* EXTRAS (Réutilisation du composant FormExtras) */}
        <FormExtras 
          extras={extras} 
