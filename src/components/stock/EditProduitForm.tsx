...
type Props = { produit: Produit; canSetIllegal: boolean };

export default function EditProduitForm({ produit, canSetIllegal }: Props) {
  ...
  async function handleSave() {
    setLoading(true);
    await fetch(`/api/produits/${produit.id}`, {
      method: "PUT",
  ...
  return (
    <div className="max-w-lg">
      <div className="mb-8">
        ...
        <p className="text-white/40 text-sm mt-1">{produit.nom}</p>
      </div>
      ...
      <div className="bg-[#16162a] border border-white/10 rounded-xl p-6 space-y-5">
        ...
        {canSetIllegal && (
          <div className="flex items-center justify-between py-1">
            <div>
              <p className="text-sm text-white/70">Produit illégal</p>
              <p className="text-xs text-white/30 mt-0.5">Ventes visibles uniquement dans Gringotts illégal</p>
            </div>
            <button
              type="button"
              onClick={() => set("illegal", !form.illegal)}
              className={`relative w-10 h-5 rounded-full transition-colors ${
                form.illegal ? "bg-red-500/70" : "bg-white/10"
              }`}
            >
              <span className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform ${
                form.illegal ? "translate-x-5" : "translate-x-0"
              }`} />
            </button>
          </div>
        )}
